...
def T_e_profile(r, z, M_500, xx = None, yy = None, cool_core = True, T_x = None):
...
def T_sz(r, z, M_500, p = "Arnaud", alpha_p_prime = False, r_max = 5, 
	 cool_core = True, norm_planck = False, T_x = None):
	...
	for x in r:
		if x <= r_max*r_500:
			nom = quad(lambda y: gnfw(None, z, M_500, p, alpha_p_prime=alpha_p_prime, xx=x, yy=y)*T_e_profile(None, z, M_500, x, y, cool_core = cool_core, T_x = T_x), 0, np.sqrt((r_max*r_500)**2-x**2))[0]
...
def T_sz_fast(r, z, M_500, p = "Arnaud", alpha_p_prime = False, r_max = 5, r_min = 1e-3, 
	      bins = 1000, cool_core = True, norm_planck = False, T_x = None):
	'''Computes the radial 2D pressure-weighted electron temperatue 
	profile of a galaxy cluster using the 3D tempeature modele 
	presented by Vikhlinin et al. (2006) and the 3D pressure 
	model given by Arnaud et al. (2010). The required value of 
	...
	for yy in y:
		if yy <= r_max:
			...
			x = np.linspace(x_min,np.sqrt(r_max**2-yy**2),bins)
			r = np.sqrt(yy**2. + x**2.) * r_500
			integrant = gnfw(r, z, M_500, p, alpha_p_prime=alpha_p_prime)*T_e_profile(r, z, M_500, cool_core = cool_core, T_x = T_x)
...
def tau_fast(r, z, M_500, p = "Arnaud", alpha_p_prime = False, r_max = 5, r_min = 1e-3, 
	...
	for yy in y:
		if yy <= r_max:
			...
			x = np.linspace(x_min,np.sqrt(r_max**2-yy**2),bins)
			r = np.sqrt(yy**2. + x**2.) * r_500
			integrant = gnfw(r, z, M_500, p, alpha_p_prime=alpha_p_prime)/(T_e_profile(r, z, M_500, cool_core = cool_core, T_x = T_x)*1000*e)
...
def simulate_rel_cluster(M_500, z, p = "Arnaud", alpha_p_prime = False, map_size = 10, pixel_size = 1.5, 
			 ...
			 cool_core = True, norm_planck = False, oversample = None, T_x = None):
	...
	y_interpol = gnfw_projected_fast(r_interpol, z, M_500, p, alpha_p_prime=alpha_p_prime, r_max=r_max, r_min=r_min, bins=bins, norm_planck=norm_planck)
	T_interpol = T_sz_fast(r_interpol, z, M_500, p, alpha_p_prime=alpha_p_prime, r_max=r_max, r_min=r_min, bins=bins, cool_core = cool_core, T_x = T_x)
	tau_interpol = tau_fast(r_interpol, z, M_500, p, alpha_p_prime=alpha_p_prime, r_max=r_max, r_min=r_min, bins=bins, cool_core = cool_core, T_x = T_x)
